...
export class PricerangeComponent implements OnInit {
  ...
  constructor (private http: HttpClient,private PricerangeService: PricerangeService) {}

  ngOnInit() {
    // this.http.get('http://localhost:4200/assets/temperatures.json').subscribe((response) => {
    ...
    console.log(JSON.stringify({ data: this.data}, null, 4));
    this.data = TEMPERATURES.map((v) => v.values.map((v) => v.date ))[0];
      this.initChart();
      this.drawAxis();
      this.drawPath();
